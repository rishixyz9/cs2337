...
double calculateArea(int arr[20][16][2], int rowIndex){
	double sum;
	int i = 1;
...
void getCoordinates(int arr[20][16][2], string coordinates, int rowIndex){
	...
	while(spaceRemover >> coordinatePair){
		value = stoi(coordinatePair.substr(0,coordinatePair.find(',')));
		arr[rowIndex][col][0] = value;
		value = stoi(coordinatePair.substr(coordinatePair.find(',') + 1));
...
void readFile(ifstream &fileInput, string fileName, int arr[20][16][2], string driverNames[20]){
	string line;
